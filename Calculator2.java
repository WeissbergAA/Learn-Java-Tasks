...
public class Calculator2 { // Объявление класса Calculator
    ...
    public static double calculate(double num1, char operator, double num2) { // Метод для выполнения операции
        double result = Double.NaN; /* Инициализация переменной результата.Double.NaN позволяет обработать случаи деления на ноль или выполнения недопустимой операции.
                                    Когда результат вычислений не определен из-за деления на ноль или из-за неверной операции (например, деление на символ или умножение на букву), возвращение Double.NaN 
                                    вместо реального числового значения помогает программе обозначить, что результат некорректен или неопределен.
...
/* Задача: Дописать калькулятор с выносом в отдельный метод в этом же классе вычислений  */
